...
$(function(){
    $('.b-header__nav').each(function(){
        var trigger = $(this).find('.js-expandedMenu__trigger');
        var expanded = $(this).find('.js-expandedMenu__expanded');
        $(expanded).hide();
        $(trigger).each(function(){
            $(this).hover(function(){
                if($(this).hasClass('active')){return false}
                    ...
                    $(this).find(expanded).slideDown('fast');
        ...
        $(this).mouseleave(hideAll);
        $(expanded).mouseleave(hideAll);
    });
//    css-columns emulation for IE9 and lower
    if(Modernizr.csscolumns){
        return false;
    }
    else{
        $('.b-footer__glossary').columnize({
            columns: 4
        });
    }
//    var expandedMenu = function(elem){
//        var trigger = $(elem).find('.js-expandedMenu__trigger');
//        var expanded = $(elem).find('.js-expandedMenu__expanded');
